...
public class Parser_With_Tree_Output {
    private Tokenizer t;
    private XMLWriter x;
     ...
     */
    Parser_With_Tree_Output(String filePath) {
    ...
    private void parseStatementBody(ASTNode parent) throws ParserException, IOException {
        if (!this.t.peekNextToken().lexeme.equals("}")) {
            while (!this.t.peekNextToken().lexeme.equals("}")) {
                this.parent = parent; // restore previous parent
                parseStatement();
            }
        }
    }
